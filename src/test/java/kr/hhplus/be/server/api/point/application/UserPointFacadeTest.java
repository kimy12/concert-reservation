...
public class UserPointFacadeTest {
    ...
    void chargePoint() {
        ...
        when(userService.findById(userId)).thenReturn(mockUser);
        when(userPointService.chargeOrDeductPoint(userId, amount, PointHistoryType.CHARGE)).thenReturn(mockUserPoint);
        ...
        verify(userService, times(1)).findById(userId);
        verify(userPointService, times(1)).chargeOrDeductPoint(userId, amount, PointHistoryType.CHARGE);
    }
    ...
    void deductPoint() {
        ...
        PointRequest.DeductPoint pointRequest = new PointRequest.DeductPoint(userId, amount, DEDUCT);

        when(userService.findById(userId)).thenReturn(mockUser);
        when(userPointService.chargeOrDeductPoint(userId, amount, DEDUCT)).thenReturn(mockUserPoint);
        ...
        verify(userService, times(1)).findById(userId);
        verify(userPointService, times(1)).chargeOrDeductPoint(userId, amount, DEDUCT);
    }
